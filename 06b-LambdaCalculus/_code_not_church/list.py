...
print('-------------MAP----------')
# MAP = lambda x:lambda f: None if x==None else (print(f'x={x} f(x)={f(x)}'),f(x))[-1] if isinstance(x,int) else (print(f'x={x} HEAD(x)={HEAD(x)}'),PAIR(MAP(HEAD(x))(f))(MAP(TAIL(x))(f)))[-1]
...
# m = MAP(r, lambda x:x*2)
"""
print(f'HEAD(m)={HEAD(m)}')
print(f'HEAD(TAIL(m))={HEAD(TAIL(m))}')
print(f'HEAD(TAIL(TAIL(m)))={HEAD(TAIL(TAIL(m)))}')
print(f'TAIL(TAIL(TAIL(m)))={TAIL(TAIL(TAIL(m)))}')
"""
EACH(m)(print)

...
def get_best_move(board, depth):
    ...
    for move in possible_moves(board):
        ...
        score = alpha_beta(board, depth - 1, False, move, float("-inf"), float("inf"))
...
# 极大极小值搜索
def alpha_beta(board, depth, maximizing_player, move, alpha, beta):
    if depth == 0 or check_win() != 0:
        return evaluate_board(board, move)
    ...
    if maximizing_player:
        ...
        for move in possible_moves(board):
            make_move(board, move, 1)
            score = alpha_beta(board, depth - 1, False, move, alpha, beta)
            max_score = max(max_score, score)
            undo_move(board, move)
        ...
        for move in possible_moves(board):
            make_move(board, move, -1)
            score = alpha_beta(board, depth - 1, True, move, alpha, beta)
            undo_move(board, move)
            min_score = min(min_score, score)

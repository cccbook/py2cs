...
def checkBatchGradient():
  ...
  for j in range(len(tocheck)):
    ...
    for i in range(mat.size):
      ...
      # print stats
      print('%s checking param %s index %s (val = %+8f), analytic = %+8f, numerical = %+8f, relative error = %+8f' % (status, name, str(np.unravel_index(i, mat.shape)), old_val, grad_analytic, grad_numerical, rel_error))

...
class SimpleRouter:
    ...
    def parse_yosys(self, netlist_str: str):
        ...
        for match in re.finditer(assign_pattern, netlist_str):
            ...
            input_signals = re.findall(r'(\w+(\[\d+\])?_*)', expression) # add by ccc
    ...
    def place_cells(self):
        ...
        self.die_width = self.die_height = math.ceil(math.sqrt(num_cells))
        print(f'num_cells={num_cells} die:width={self.die_width}')
        # 計算網格的邊長
        ...
        # print(f'grid_width={grid_width}, grid_height={grid_height} grid_size={grid_size}')
        # 遍歷所有需要放置的單元
        for cell in self.cells.values():
            ...
            # 如果到達當前行的末尾
            # 則移動到下一行的開始位置
            if current_x >= self.die_width: #grid_size:  # 若已到達行尾
    ...
    def route_nets(self):
        ...
        for cell in self.cells.values():
            ...
            for pin in cell.pins:
                ...
                # 只處理輸出管腳
                if pin.is_output:
                    # 找出所有需要連接到這個輸出管腳的輸入管腳 ??? ccc
                    matching_inputs = [
                        (target_cell, target_pin) 
                    ...
                    print(f'matching_inputs = {matching_inputs}')
                    
                    # 根據輸入數量計算偏移
                    for idx, (target_cell, target_pin) in enumerate(matching_inputs):
                        # 計算偏移量 - 使用索引值創建微小偏移
                        # 水平和垂直方向都添加輕微偏移
                        offset_x = idx * dx  # 水平方向偏移
                        offset_y = idx * dy  # 垂直方向偏移
                        
                        # 創建帶偏移的曼哈頓路由
                        route = self.create_manhattan_route(
                            (cell.x + offset_x, cell.y + offset_y),     # 起點：輸出管腳所在單元的位置（添加偏移）
    ...
    def generate_def(self) -> str:
        ...
        for route_info in routes:
            # 使用 output_pin 作為網路名稱
            net_name = route_info['output_pin']
            ...
            path_str = " ".join([
                f"( {int(x)} {int(y)} )" 
                for x, y in route_info['route']
            ])
            
            def_content.append(f"  + ROUTED {path_str} ;")
            
            # 可選：如果需要顯示偏移信息，可以添加備註
            def_content.append(f"  # Offset: {route_info['offset']}")

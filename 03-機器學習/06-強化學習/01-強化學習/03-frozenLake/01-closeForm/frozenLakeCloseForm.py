...
import itertools
import numpy as np
import gymnasium as gym  # Changed gym to gymnasium with alias
...
# 获取未包装的环境（unwrapped environment）以访问 P 属性
...
class ClosedFormAgent:
        ...
        for t in range(env.spec.max_episode_steps - 1, -1, -1):
            for s in range(state_n):
                for a in range(action_n):
                    for p, next_s, r, d in raw_env.P[s][a]:
    ...
    def step(self, observation, reward, terminated):
        action = int(self.pi[self.t, observation])  # Cast to int to handle numpy int64
        self.t += 1
        return action
...
def play_episode(env, agent, seed=None, mode=None, render=False):
    observation, info = env.reset(seed=seed)  # Changed to handle info return
    ...
    while True:
        ...
        if terminated or truncated:
            break
        observation, reward, terminated, truncated, info = env.step(action)  # Handle info

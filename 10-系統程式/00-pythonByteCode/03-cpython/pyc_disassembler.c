...
// 這段有錯
    ...
    for (Py_ssize_t i = 0; i < bytecode_size; ) {
        ...
        // printf("%4zd: %s", i, _PyOpcode_OpName[opcode]);  // 使用 Python 的操作碼名稱 (這個在 internal/pycore_opcode_metadata 引用不到)
        printf("%4zd: %x", i, opcode);  // 使用 Python 的操作碼名稱 (這個在 internal/pycore_opcode_metadata 引用不到)
/*
        // 處理操作數
        int oparg = 0;

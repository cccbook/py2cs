...
class Net(nn.Module):
	...
	def exist(self):
		filename = self.hash()
		return os.path.isfile(f"output/{filename}.json")

	def load(self):
		filename = self.hash()
		jsonFile = open(f"output/{filename}.json", "rt")
		jsonStr = jsonFile.read()
		jsonObj = json.loads(jsonStr)
		jsonFile.close()
		self.model["accuracy"] = jsonObj["accuracy"] # 取得正確率
		# torch.load(net.state_dict(), f"output/{filename}.pt")
		return jsonObj

	def save(self):
		filename = self.hash()
		jsonFile = open(f"output/{filename}.json", "wt")
		jsonFile.write(json.dumps(self.model, indent=2))
		jsonFile.close()
		torch.save(self.net.state_dict(), f"output/{filename}.pt")

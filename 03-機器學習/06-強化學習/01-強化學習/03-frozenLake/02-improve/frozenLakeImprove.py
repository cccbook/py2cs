...
# 獲取未包裝的環境（unwrapped environment）以訪問 P 屬性
...
logging.info('P[14] = %s', raw_env.P[14])  # 使用 raw_env 訪問 P
logging.info('P[14][2] = %s', raw_env.P[14][2])  # 使用 raw_env 訪問 P
...
def play_policy(env, policy, render=False):
    episode_reward = 0.
    observation, _ = env.reset()  # gymnasium 返回 (observation, info)
    while True:
        if render:
            env.render()  # render the environment
        action = np.random.choice(env.action_space.n, p=policy[observation])
        observation, reward, terminated, truncated, _ = env.step(action)  # gymnasium 返回 5 個值
        episode_reward += reward
        if terminated or truncated:  # 同時檢查 terminated 和 truncated
            break
...
def v2q(env, v, state=None, gamma=1.):  # calculate action value from state value
    raw_env = env.unwrapped  # 獲取未包裝的環境
    
    if state is not None:  # solve for single state
        q = np.zeros(env.action_space.n)
        for action in range(env.action_space.n):
            for prob, next_state, reward, terminated in raw_env.P[state][action]:  # 使用 raw_env.P
                q[action] += prob * \

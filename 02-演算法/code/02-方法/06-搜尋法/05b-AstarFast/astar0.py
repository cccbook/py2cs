...
class App:
    ...
    def a_star_algorithm(self, start, goal):
        ...
        for i in range(25):
            ...
            for j in range(25):
                temp = float('inf')
                came_from[i].append(()) # came_from[i].append([])
        ...
        while len(open_set) > 0:
            ...
            f_current = heapq.heappop(f_open_set)
            current = f_current[1]
            ...
            for node in neighbors:      # 對於每個鄰居
                ...
                if tentative_gScore < g_score[node_row][node_column]: # 如果從 current 到 node 的新走法 gScore 更低
                    ...
                    # came_from[node_row][node_column].append(current_row) # 更新 node 的 came_from 為 current
                    # came_from[node_row][node_column].append(current_column)
                    came_from[node_row][node_column] = (current_row, current_column)
                    ...
                    if node not in open_set: # 若 node 不在 openSet (藍點) 中
                        ...
                        open_set.add(node) # open_set.append(node[:]) # 將 node 加入 openSet 中
